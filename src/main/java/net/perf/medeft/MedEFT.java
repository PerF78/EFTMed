...
public class MedEFT {
    ...
    private void handleEndermanAttack(Player player, float damage) {
        if (Math.random() < 0.5) {
            updateLimbHealthWithArmor(player, "head_health", damage * 0.6f);
        } else {
    ...
    private void handleArrowAttack(Player player, float damage, double arrowY, DamageSource source) {
        ...
        if (yDifference > 1.5) {
            if (player.getInventory().armor.get(3).isEmpty()) { // проверяем надет ли шлем
                updateLimbHealthWithArmor(player, "head_health", damage * 20.0f);
    ...
    private void updateLimbHealth(Player player, String limb, float damage) {
        ...
        if (currentHealth > 0) {
            ...
            if (limb.equals("head_health") && (newHealth <= 0)) {
                player.getPersistentData().putFloat("body_health", 0.0f);
                player.getPersistentData().putFloat("arms_health", 0.0f);
                player.getPersistentData().putFloat("legs_health", 0.0f);
    ...
    private static void updatePlayerHealth(Player player) {
        ...
        Objects.requireNonNull(player.getAttribute(Attributes.MAX_HEALTH)).setBaseValue(totalHealth);
